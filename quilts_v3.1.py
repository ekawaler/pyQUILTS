...
def filter_known_transcripts(transcriptome_bed, results_folder, logfile):
	...
	if f:	
		...
		for line in f.readlines():
			...
			#strip special characters and trailing commas at the end of every line to avoid introducing empty string entries
			block_sizes=block_sizes.strip().strip(",")
			block_starts=block_starts.strip().strip(",")
