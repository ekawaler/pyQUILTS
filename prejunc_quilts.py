...
def merge_junction_files_old(junc_dir, log_dir):
	'''Merges junction files found in junc_dir. Returns 1 if unable to open any junction files or nothing if it runs to completion. I think this is all wrong so I'm deprecating it for now'''
...
def merge_junction_files(junc_dir, log_dir):
		...
		for line in f.readlines():
			...
			if len(spline) > 11:
				...
				if splen_exons[0] >= min_length and splen_exons[1] >= min_length and sum(splen_exons) >= min_sum_length:
					...
					exon_len_map[chrm][begin_intron][end_intron] = [splen_exons, spbegin_exons]
	...
	for chr in sorted(num_observ_map.keys()):
		for beg_int in sorted(num_observ_map[chr].keys()):
			for end_int in sorted(num_observ_map[chr][beg_int].keys()):
				[lens, begins] = exon_len_map[chr][beg_int][end_int]
				begin_ex_1 = beg_int-lens[0]-1
				end_ex_2 = end_int+lens[1]
				start_ex_2 = end_int+1-begin_ex_1
				if start_ex_2 > 0:
					w.write("%s\t%d\t%d\tj%d\t%d\t+\t%d\t%d\t0\t2\t%d,%d\t%d,%d\n" % (chr,begin_ex_1,end_ex_2,count,num_observ_map[chr][beg_int][end_int],begin_ex_1,end_ex_2, lens[0], lens[1], begins[0], begins[1]))
					count += 1
	w.close()
...
def filter_known_transcripts(transcriptome_bed, results_folder, logfile):
	...
	if f:	
		...
		for line in f.readlines():
			...
			for i in range(0,block_count-1):
				begin_intron = start+sizes[i]
				end_intron = begin_exon+starts[i+1]+1
				if spline[3] == 'NM_207014':
					print spline[3], begin_intron, end_intron
	...
	w2 = open(results_folder+'/merged-junctions.known.bed','w')
	...
	for line in f.readlines():
		...
		if len(spline) > 11:
			...
			if num_observ >= 1:	
				begin_intron = begin_ex_1 + splen_exons[0] + 1
				end_intron = end_ex_2 - splen_exons[-1] + 1
				...
				# Log it
				if junction_status == '':
					print "NO MATCH", line, begin_intron, end_intron
					w.write(line)
				else:
					print "YES MATCH", line, begin_intron, end_intron
...
def filter_alternative_splices(log_dir, threshA, threshAN, threshN, logfile):
	...
	#f = open('/ifs/data/proteomics/tcga/databases/refseq_human_20160914/transcriptome.bed','r')
