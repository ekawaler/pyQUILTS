...
def write_peptides(gene, vars, peptide_start_pos, peptide, out_file):
	...
	for var_set in var_combos:
		...
		tryp_pep = trypsinize(pep_to_write)
		if len(tryp_pep) >= 2:
			write_to_status('%s\t%s\%s\n' % (pep_to_write, str(tryp_pep), str(var_set)))
		# Letting there be a missed cleavage just in case the variant changes the cleaving.
		for pept in tryp_pep:
			if pept not in existing_peps:
				if 25 >= len(pept) >= 6:
				...
				existing_peps.append(pept)
				new_peptides.append([pept, var_string])
	return new_peptides
...
def assign_variants(gene, tryptic_peptides, variants, out_file, no_missed_cleavage):
	...
	for i in range(len(tryptic_peptides)-1):
		...
		for var in vars.keys():
			if vars[var][0][0] in '*RK' and var-1==total_aas+len(peptide):
				vars_in_peptide.append(var)
				peptide += vars[var][0][0]+tryptic_peptides[i+1]
